...
namespace AquariumGame._0.GameEnvironment
{
    public class Water
    {
        ...
        public float Dampening = 0.0025f;
        ...
        private void splashingThreads()
        {
            int index = ScreenManager.gameThread.Count;
            if (index < 2)
            ScreenManager.gameThread.Add(Task.Factory.StartNew(() => Splash(index, Misc.generaterValue(1, 2000, 12000) * (index + 1), Misc.generaterValue(1, 100, 200) * (index + 1), 30)));
        }

        // Returns the height of the water at a given x coordinate.
        public float GetHeight(float x)
        {
            if (x < 0 || x > Screens.ScreenManager.DEFAULT_SCREEN_WIDTH)
                return 100;
        ...
        public void Splash(int threadIndex,int timer, float xPosition, float speed)
        {
            ...
            Thread.Sleep(timer);
            //splashThread[threadIndex].Abort();
        }
        ...
        public void Draw()
        {
            ...
            ScreenManager.Sprites.Draw(particlesTarget, -Vector2.One, new Color(0.8f, 0.8f, 1f));
            ScreenManager.Sprites.Draw(particlesTarget, Vector2.One, new Color(0f, 0f, 0.2f));
            ScreenManager.Sprites.Draw(particlesTarget, Vector2.Zero, lightBlue);
            ScreenManager.Sprites.End();
            // draw the waves
            pb.Begin(PrimitiveType.TriangleList);
            Color midnightBlue = new Color(0, 15, 40) * 0.9f * Sun.sunRise;
            lightBlue *= 0.8f;
